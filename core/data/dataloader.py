...
# test
# def get_dataloader(config, mode, model_type, distribute, signal):
# train
def get_dataloader(config, mode, model_type, distribute):
    ...
    # if signal == "evolve":
    #     config["data_root"] = "/home/inspur/MAX_SPACE/Jiyu/LFSLog/dataset_floder/standard/evolve1/bgl_session"
    # else:
    #     pass
    assert (
    ...
    if mode == "train" and config["augment"]:
        ...
        elif config["image_size"] == 300:
            trfms_list.append(transforms.Resize((300, 300)))
        ...
        # for MTL -> alternative solution: use avgpool(ks=11)
            ...
            trfms_list.append(transforms.Resize((92, 92)))
            trfms_list.append(transforms.CenterCrop((80, 80)))
        elif config["image_size"] == 18:
            trfms_list.append(transforms.Resize((18, 18)))
        else:
            raise RuntimeError

    trfms_list.append(transforms.ToTensor())
    # trfms_list.append(transforms.Normalize(mean=[0.5], std=[0.5]))
