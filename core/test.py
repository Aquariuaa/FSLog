...
class Test(object):
    ...
    def __init__(self, rank, config, result_path=None):
        ...
        # self.test_loader, self.test_loader2 = self._init_dataloader(config)
    ...
    def test_loop(self):
        ...
        for epoch_idx in range(self.config["test_epoch"]):
            ...
            _, accuracies, ress = self._validate(epoch_idx)
            pre, rec, f1 = self.PRF(ress)
            test_accuracy, h = mean_confidence_interval(accuracies)
            # print("Test Accuracy: {:.3f}\t h: {:.3f}".format(test_accuracy, h))
            print("Test Precision: {:.3f}\t Recall: {:.3f}\t F1-measure: {:.3f}".format(pre*100, rec*100, f1*100))
            total_accuracy += test_accuracy
            total_precision += pre*100
            total_recall += rec*100
            total_f1 += f1*100
        ...
        print("Aver Precision: {:.3f}".format(total_precision/self.config["test_epoch"]))
        print("Aver Recall: {:.3f}".format(total_recall/self.config["test_epoch"]))
        print("Aver F1-measure: {:.3f}".format(total_f1/self.config["test_epoch"]))
    ...
    def _validate(self, epoch_idx):
        ...
        # with torch.no_grad():
            ...
            batches = []
            # for batch_idx2, batch2 in enumerate(zip(*loader2)):
            ...
            for batch_idx, batch in enumerate(zip(*loader)):
                ...
                elem1 = [elem for each_batch in batch for elem in each_batch]
                # elem2 = [elem2 for each_batch2 in batches[batch_idx] for elem2 in each_batch2]

                # output, acc, res = self.model([elem1,elem2])
                output, acc, res = self.model(elem1)
        ...
        if self.distribute:
            ...
            self.model.reverse_setting_info()
        return meter.avg("acc"), accuracies, ress
    ...
    def PRF(self,ress):
        ...
        for res in ress:
            ...
            tn = tn + res[2]
            fn = fn + res[3]
        print("total_numbers: {:.3f}".format(tp+fp+tn+fn))
        pre = tp/(tp+fp)
        rec = tp/(tp+fn)
    ...
    def _init_files(self, config):
        ...
        state_dict_path = os.path.join(result_path, "checkpoints", "model_00001.pth")
    ...
    def _init_dataloader(self, config):
        ...
        # test_loader = get_dataloader(config, "test", self.model_type, distribute, "origin")
        # test_loader2 = get_dataloader(config, "test", self.model_type, distribute, "origin")
